...
export default function ProductsClient({ products }: { products: ProductWithDenominations[] }) {
  ...
  return (
    <>
      <main className="min-h-screen pt-24 tablet:pt-28 pb-24 relative overflow-hidden bg-background">
        <div className="container-app relative z-10">
          {/* Page Header */}
          <div className="mb-8 tablet:mb-10 text-center tablet:text-left">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-500/10 border border-brand-500/20 mb-4 shadow-sm mx-auto tablet:mx-0">
              <Cpu className="w-3.5 h-3.5 text-brand-500" />
              <span className="text-[11px] font-bold tracking-widest text-brand-500 uppercase font-heading">Nexa Digital Market</span>
            </div>
            <h1 className="heading-1 mb-2">
              Katalog <span className="text-brand-500">Produk Digital</span>
            </h1>
          ...
          <div className="mb-8">
            <div className="relative mb-6 group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-brand-500 transition-colors z-10 pointer-events-none" />
              <input
                type="text"
                placeholder="Cari produk digital... (contoh: Netflix, Telkomsel, Token PLN, Google Play)"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-[#121620] border border-white/10 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/25 transition-all"
              />
            </div>
              ...
              {allCategories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'relative flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-colors whitespace-nowrap border',
                      isActive
                        ? 'text-white border-transparent'
                        : 'text-muted-foreground border-white/10 bg-[#121620] hover:text-foreground hover:border-white/20'
                    )}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="activeProductCatPill"
                        className="absolute inset-0 bg-brand-500 rounded-full z-0 shadow-sm"
          ...
          {/* Product Grid — Unified GameCard Design (6 Cols) */}
