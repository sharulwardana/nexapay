        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-16 right-3 w-[calc(100vw-24px)] max-w-sm tablet:absolute tablet:top-full tablet:right-0 tablet:left-auto tablet:w-[360px] tablet:mt-2 glass-card border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 max-h-[70vh] flex flex-col"
            >
              <div className="p-4 border-b border-border/60 flex justify-between items-center bg-muted/30 flex-shrink-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{getItemCount()} item tersimpan</p>
                </div>
                <button onClick={onToggle} className="p-1.5 rounded-xl bg-muted text-muted-foreground">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {cartItems.length > 0 && (
                <div className="p-3.5 border-t border-border/60 bg-background/90 backdrop-blur-md flex-shrink-0">
                  ...
                  <Link
                    href="/checkout"
                    onClick={() => { playClick(); closeAll(); }}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl gradient-primary text-white font-bold text-xs shadow-neon-violet"
                  >
